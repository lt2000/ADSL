...
def write(backend, storage, bucket_name, mb_per_file, number, key_prefix,write_times):
    ...
    def write_object(key_name, storage):
        ...
        for unused in range(write_times):
            storage.put_object(bucket_name, key_name, d)
            bytes_write += bytes_n
        end_time = time.time()

        mb_rate = bytes_write/(end_time-start_time)/1e6
...
@click.option('--write_times', default=1, help="number of times to write each COS key")
def write_command(backend, storage, bucket_name, mb_per_file, number, key_prefix, outdir, name, write_times):
    if name is None:
        name = number
    if bucket_name is None:
        raise ValueError('You must provide a bucket name within --bucket_name parameter')
    res_write = write(backend, storage, bucket_name, mb_per_file, number, key_prefix, write_times)
...
@click.option('--read_times', default=1, help="number of times to read each COS key")
@click.option('--write_times', default=1, help="number of times to write each COS key")
def run(backend, storage, bucket_name, mb_per_file, number, key_prefix, outdir, name, read_times, write_times):
    if name is None:
        name = number
